...
## 1. User's device or chat attachment → `attach_media` (hosted connector only)

**ALWAYS** use `attach_media` when the user wants to post a photo, video, or
PDF from their device or attached in the chat. It opens an upload widget (drop
...
quality is preserved end-to-end.

- Limits: images to 20MB, videos to 500MB, PDFs to 100MB.
- When the upload finishes, the media's url and type arrive in your context
  (and via `list_media`), ready for `create_post`'s `mediaItems`.
...
- **NEVER read and base64-encode an attached file yourself.** Chat channels
  ...
  becomes a 6KB thumbnail). An upload widget DOES exist — never tell the user
  it doesn't.

No `attach_media` (stdio server / CLI)? Have the user upload once on the
dashboard Media page (https://schedulala.com/dashboard), then find the file
with `list_media` and use its url.
